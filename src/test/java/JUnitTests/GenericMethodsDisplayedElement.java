...
public class GenericMethodsDisplayedElement {
	WebDriver driver;
	
	public GenericMethodsDisplayedElement(WebDriver driver) {
		this.driver = driver;
	}
	...
	public void clickWhenReady(By locator, int timeout) {
		WebElement element = null;
		try {
			System.out.println("Waiting for max:: " + timeout + " seconds for element to be clicable");

			WebDriverWait wait = new WebDriverWait(driver, 3);
			element = wait.until(
					ExpectedConditions.elementToBeClickable(locator));
			element.click();
			System.out.println("Element click on the web page");
		} catch(Exception e) {
			System.out.println("Element not appeared on the web page");
		}
	}
